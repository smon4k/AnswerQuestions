  ...
  <div class="container">
    ...
    <div class="input" v-show="pageState == 3">
      <van-row>
        <van-col :span="5">
          <van-button class="input-b">
            <div class="input-b-box" v-if="exchangeArray.INPUT !== '' && exchangeArray.INPUT >= 0">
              ...
              <!-- <i class="van-icon-arrow-down van-icon--down"></i> -->
            </div>
        ...
        <van-col :span="19" class="textRight">{{ $t('swap:Balance') }}： {{currentPools && currentPools.tokenBalance && computeTokenBalanceChange(currentPools.tokenBalance, currentPools.tokenDecimals, 10)}}</van-col>
      </van-row>
      <van-row class="input-box">
        <van-col :span="24">
          ...
          <van-button 
            ...
            ) ">{{ $t('swap:Max') }}</van-button>
        </van-col>
      </van-row>
      <div class="arrow">
          <van-button size="small" circle>
    ...
    <div class="input">
      <van-row>
        <van-col :span="12">
          <van-button class="input-b" @click="handleClickOpen('INPUT')" :disabled="pageState == 3 ? true : false">
            <div class="input-b-box" v-if="exchangeArray.INPUT !== '' && exchangeArray.INPUT >= 0">
              <img size="small" :src="getFilersSwapPoolsArr(exchangeArray.INPUT).logo" width="20" />
              <span>{{getFilersSwapPoolsArr(exchangeArray.INPUT).name}}</span>
              <i v-show="pageState !== 3" class="van-icon-arrow-down van-icon--down"></i>
            </div>
            <div class="input-b-box" v-else>
              <span>{{ $t('swap:SelectCurrency') }}</span> 
              <!-- <i class="van-icon-arrow-down van-icon--down"></i> -->
        ...
        <van-col :span="12" class="textRight">{{ $t('swap:Balance') }}： {{balanceTransformation(getFilersSwapPoolsArr(exchangeArray.INPUT).tokenBalance)}}</van-col>
      </van-row>
      <van-row class="input-box">
        <van-col :span="24">
          <van-field
          ...
          <van-button 
            ...
            size="small" 
            round 
            @click="inputChangeValue(keepDecimalNotRounding(getFilersSwapPoolsArr(exchangeArray.INPUT).tokenBalance))">{{ $t('swap:Max') }}</van-button>
        </van-col>
      </van-row>
    </div>

    <div class="arrow" v-if="pageState == 1">
      <van-button size="small" circle @click="switchCurrencyClick">
        <!-- <i class="van-icon-sort"></i> -->
        <van-icon name="sort" />
      </van-button>
    </div>
    <div class="arrow" v-else>
      <van-button size="small" circle>
        <i class="van-icon-plus"></i>
      </van-button>
    </div>
    ...
    <div class="output">
      <van-row>
        <van-col :span="12">
          <van-button class="input-b" @click="handleClickOpen('OUTPUT')" :disabled="pageState == 3 ? true : false">
        ...
        <van-col :span="12" class="textRight">{{ $t('swap:Balance') }}： {{balanceTransformation(getFilersSwapPoolsArr(exchangeArray.OUTPUT).tokenBalance)}}</van-col>
      </van-row>
      <van-row class="input-box">
        <van-col :span="24">
          <van-field
    ...
    <van-dialog
      ...
      <div>
        ...
        >
          ...
          <van-table-column property="name" width="100"></van-table-column>
          <van-table-column>
            <template slot-scope="scope">
      ...
      <!-- <span slot="footer" class="dialog-footer">
        ...
        <van-button type="primary" @click="dialogVisible = false">确 定</van-button>
      </span> -->
    </van-dialog>
  </div>
</template>
...
.container {
  ...
  .dialogClass {
    /deep/ {
      ...
      .van-table .select-row {
        // background: rgba(255, 255, 255, 0.16);
        @include selectSwapTokenBgc($selectSwapToken-dark);
        cursor:pointer;
        // background: #909399;
      }
      .van-table__body-wrapper {
        @include sideBarSwapInputBgc($claimCardSwapInput-light);
      }
      .van-table--enable-row-hover .van-table__body tr:hover>td {
        // background-color: rgba(255, 255, 255, 0.16);
        @include selectSwapTokenBgc($selectSwapToken-dark);
        // @include sideBarSwapInputBgc($claimCardSwapInput-light);
        @include mainFont($color-mainFont-light);
      }
      // .van-table__body tr.current-row>td.van-table__cell {
      //   background-color: rgba(255, 255, 255, 0.16);
      // }
      .van-table td.van-table__cell {
        border-bottom: 0;
      }
      .van-table::before {
        z-index: 0;
