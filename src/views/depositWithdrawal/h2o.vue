                ...
                <van-tabs v-model="activeName" @click="handleClick" :stretch="true">
                    <van-tab title="存入" name="1" :disabled="trading">
                        <van-row class="balance">
                            <van-col :span="24">
                                <div>
                                    <span>平台余额：{{ keepDecimalNotRounding(Number(localBalance) + Number(walletBalance), 4) }} H2O</span>
                                    <br />
                                    <span>钱包余额：{{ toFixed(Number(H2OBalance), 4) }} H2O</span>
                    ...
                    <van-tab title="提取" name="2" :disabled="trading">
                        <van-row class="balance">
                            <van-col :span="24">
                                <div>
                                    <span>平台余额：{{ toFixed(Number(localBalance) + Number(walletBalance), 4) }} H2O</span>
                                    <br />
                                    <span>钱包余额：{{ toFixed(Number(H2OBalance), 4) }} H2O</span>
                                    <!-- <span>GS Balance：{{gsBalance}}</span> -->
                                </div>
...
export default {
  ...
  computed: {
      ...mapState({
        ...
        gamesFillingAddress:state=>state.base.h2oFillingAddress,
  ...
  methods: {
    ...
    async refreshData() {
        ...
        this.timeInterval = setInterval(async() => {
            ...
            this.H2OBalance = await getBalance(Address.H2O, 18); //获取H2O余额
    ...
    async getIsApprove() { //获取余额 查看是否授权
      let balance = await getBalance(Address.H2O, 18); //获取余额
      console.log("H2O balance", balance);
      this.tokenBalance = balance;
      isApproved(Address.H2O, 18, balance, this.gamesFillingAddress).then((bool) => {
    ...
    startApprove() { //批准USDT
      this.trading = true;
      approve(Address.H2O, this.gamesFillingAddress).then((hash) => {
        // console.log(result);
        if(hash) {
          this.approve = true;
          this.trading = false;
        ...
        this.$refs[formName].validate().then(async ()=>{
          if (!this.trading) {
            ...
            const fillingRecordParams = { //充提记录参数
                ...
                h2o_local_balance: this.localBalance,
                h2o_wallet_balance: this.walletBalance,
                hash: '',
                currency: 'h2o'
            };
            contractName(amount, Address.H2O, this.gamesFillingAddress, 18, fillingRecordParams, 2).then(async (hash) => {
                if(hash) {
                    if(this.activeName == 1) {//充值的话 二次检测是否充值成功
                        // await this.setDepositWithdraw(amount, hash);
        ...
        await axios.post(this.apiUrl + '/Api/Depositwithdrawal/depositWithdraw', {
            ...
            if (json && json.code == 10000) {
                ...
                if(this.activeName == 1) {
                    ...
                    saveNotifyStatus(0, true, 'h2o');
                } else {
                    saveNotifyStatus(0, false, 'h2o'); //提取的话 这里不通知GS获取余额
                }
                this.trading = false;
    ...
    async getGameFillingBalanceFun(deWithId, hash) { //2. 获取合约中充提绝对余额 检测是否充提成功
        ...
        let depositTimer = setInterval(async () => {
            ...
            if(receipt && receipt.status) {
                await setDepWithdrawStatus(deWithId, 2, true, 'h2o'); //修改充值状态为已完成 并通知GS获取余额
                // this.trading = false;
        ...
        let withdrawTimer = setInterval(async () => {
            if(withdrawId) {
                ...
                if(withdrawStatus) {
                    ...
                    saveNotifyStatus(0, true, 'h2o'); //通知GS更新余额
                    setTimeout(async () => {
    ...
    async getUserInfo(isHint=false) { //获取用户信息
        axios.get(this.apiUrl + "/Api/Depositwithdrawal/getFillingRecordUserInfo", {
            ...
            console.log(json);
            if (json.code == 10000) {
                this.localBalance = keepDecimalNotRounding(json.data.h2o_local_balance, 4, true);
