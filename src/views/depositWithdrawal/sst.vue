                        ...
                        <van-tabs v-model="activeName" @click="handleClick" :stretch="true">
                            <van-tab title="存入" name="1" :disabled="trading">
                                <van-row class="balance">
                                    <van-col :span="24">
                                        <div>
                                            <span>平台余额：{{ keepDecimalNotRounding(Number(localBalance) + Number(walletBalance), 4) }} SST</span>
                                            <br />
                                            <span>钱包余额：{{ toFixed(Number(SSTBalance), 4) }} SST</span>
                            ...
                            <van-tab title="提取" name="2" :disabled="trading">
                                <van-row class="balance">
                                    <van-col :span="24">
                                        <div>
                                            <span>平台余额：{{ toFixed(Number(localBalance) + Number(walletBalance), 4) }} SST</span>
                                            <br />
                                            <span>钱包余额：{{ toFixed(Number(SSTBalance), 4) }} SST</span>
                                            <!-- <span>GS Balance：{{gsBalance}}</span> -->
                                        </div>
...
export default {
  ...
  data() {
    return {
        ...
        actions: [{ name: 'SST' }, { name: 'USDT' }],
        selectCurrencyShow: false,
        currencyName: 'SST', //默认币 SST
  ...
  computed: {
      ...mapState({
        ...
        gamesFillingAddress:state=>state.base.sstFillingAddress,
  ...
  methods: {
    ...
    async refreshData() {
        ...
        this.timeInterval = setInterval(async() => {
            ...
            this.SSTBalance = await getBalance(Address.SST, 18); //获取SST余额
    ...
    async getIsApprove() { //获取余额 查看是否授权
      let balance = await getBalance(Address.SST, 18); //获取余额
      console.log("SST balance", balance);
      this.tokenBalance = balance;
      isApproved(Address.SST, 18, balance, this.gamesFillingAddress).then((bool) => {
    ...
    startApprove() { //批准USDT
      this.trading = true;
      approve(Address.SST, this.gamesFillingAddress).then((hash) => {
        // console.log(result);
        if(hash) {
          this.approve = true;
          this.trading = false;
    ...
    async getGameFillingBalanceFun(deWithId, hash) { //2. 获取合约中充提绝对余额 检测是否充提成功
        ...
        let depositTimer = setInterval(async () => {
            ...
            if(receipt && receipt.status) {
                await setDepWithdrawStatus(deWithId, 2, true, 'sst'); //修改充值状态为已完成 并通知GS获取余额
                // this.trading = false;
        ...
        let withdrawTimer = setInterval(async () => {
            if(withdrawId) {
                ...
                if(withdrawStatus) {
                    ...
                    saveNotifyStatus(0, true, 'sst'); //通知GS更新余额
                    setTimeout(async () => {
    ...
    async getUserInfo(isHint=false) { //获取用户信息
        axios.get(this.apiUrl + "/Api/Depositwithdrawal/getFillingRecordUserInfo", {
            ...
            if (json.code == 10000) {
                ...
                this.SSTBalance = await getBalance(Address.SST, 18); //获取SST余额
                console.log('SST 余额：', this.SSTBalance);
            } else {
