                            ...
                            <van-tab :title="$t('question:deposit')" name="1" :disabled="trading">
                                <van-row class="balance">
                                    <van-col :span="24">
                                        <div>
                                            <!-- 平台余额： -->
                                            <span>{{ $t('question:PlatformBalance') }}：{{ toFixed(Number(localBalance) + Number(walletBalance), 4) }} SCT</span>
                                            <br />
                                            <!-- 钱包余额： -->
                                            <span>{{ $t('question:WalletBalance')  }}：{{ toFixed(Number(SCTBalance), 4) }} SCT</span>
  ...
  methods: {
    ...
    allWalletBlanceFun() { //全部钱包余额
        if(this.SCTBalance > 0) {
            this.depositForm.amount = Math.trunc(this.SCTBalance);
            return true;
        }
        return false;
    },
